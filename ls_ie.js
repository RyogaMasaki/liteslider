...
window.now=(function() {
  ...
  else return performance.now.bind(performance);
})();
var lastTime=0;
...
var _liteSlider={
    ...
    ls._.curr.style.filter = 'alpha(opacity=' + (1-perc)*100 + ')';
		ls._.next.style.opacity=perc;
		ls._.next.style.filter = 'alpha(opacity=' + perc*100 + ')';
	},
  // Reset metrics
	resetMx:function(ls) {
...
var liteSlider=function(element,settings){
	...
	switch(typeof element) {
		case 'string':
			...
			if(this._.slider===null) throw "liteSlider: Unable to find the element by the given ID";
			break;
		case 'object':
			if(!element.hasOwnProperty('Node')) throw "liteSlider: Object is not a DOM element (no Node property detected)";
			this._.slider=element;
			break;
		case 'undefined':
			throw "liteSlider: Element parameter is undefined";
		default:
			throw "liteSlider: Invalid type passed for element";
	}
